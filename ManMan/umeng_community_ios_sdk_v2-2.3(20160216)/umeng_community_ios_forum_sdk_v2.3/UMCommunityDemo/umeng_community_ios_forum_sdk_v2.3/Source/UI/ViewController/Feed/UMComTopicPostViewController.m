...
- (void)viewWillLayoutSubviews
{
    ...
    if (!_menuView) {
        ...
        collectionMenuView.dropMenuViewWidth = 90;
...
- (void)horizonCollectionView:(UMComHorizonCollectionView *)collectionView reloadCell:(UMComHorizonCollectionCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        ...
        imageFrame.size.height = UMComWidthScaleBetweenCurentScreenAndiPhone6Screen(6.f);
...
{
    UIViewController *viewController = self.childViewControllers[self.menuView.currentIndex];
    CGRect frame = viewController.view.frame;
    frame.size.height = self.view.frame.size.height - self.menuView.frame.size.height;
